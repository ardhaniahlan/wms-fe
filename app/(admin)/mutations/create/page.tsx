...
export default function CreateMutationPage() {
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        const [itemsData, racksData] = await Promise.all([getItems(), getRacks()]);
        setItems(itemsData);
        setRacks(racksData);
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        destinationRackId: formData.type === 'OUT' ? null : formData.destinationRackId,
      };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center mb-6">
        <Link href="/mutations" className="text-slate-500 hover:text-blue-600 mr-4">&larr; Kembali</Link>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Tipe Mutasi</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Kuantitas (Qty)</label>
              <input
          ...
          <div>
            ...
            <select
              required className="w-full px-4 py-2 border rounded-lg outline-none focus:border-blue-500"
          ...
          {(formData.type === 'OUT' || formData.type === 'TRANSFER') && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Dari Rak (Asal)</label>
              <select
                required className="w-full px-4 py-2 border rounded-lg outline-none focus:border-blue-500"
                value={formData.sourceRackId} onChange={(e) => setFormData({ ...formData, sourceRackId: e.target.value })}
              >
                <option value="" disabled>-- Pilih Rak Asal --</option>
                {racks.map((rack) => (
                  <option key={rack.id} value={rack.id}>{rack.code} ({rack.warehouse?.name})</option>
                ))}
              </select>
            </div>
          )}
          ...
          {(formData.type === 'IN' || formData.type === 'TRANSFER') && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Ke Rak (Tujuan)</label>
              <select
                required className="w-full px-4 py-2 border rounded-lg outline-none focus:border-blue-500"
                value={formData.destinationRackId} onChange={(e) => setFormData({ ...formData, destinationRackId: e.target.value })}
              >
          ...
          <div>
            ...
            <input
              type="text" placeholder="No. Surat Jalan / Alasan Mutasi"
              className="w-full px-4 py-2 border rounded-lg outline-none focus:border-blue-500"
          ...
          <div className="pt-4 flex justify-end">
            <button type="submit" disabled={isLoading} className="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 disabled:bg-blue-400">
